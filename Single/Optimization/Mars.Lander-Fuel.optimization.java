// Mars Lander - Fuel Optimization (1991)
import java.util.*;
...
class Player {
    ...
    private static int getRotation (int R, int VS, int HS) {
        ...
        if (shouldKeepAltitude(X, Y) && HS != 0) {  // If keeping altitude has priority, lander should stand
            ...
            if (Math.abs(HS) > 7 && Y > siteY + 100) {  // Compensating for h.speed only if landing site isn't nearby 
    ...
    private static int getPower(int R, int VS, int HS) {
        ...
        if (getDistance(X) == 0 && R == 0 && (Y - siteY) < 123 && VS > -30) {    // Switch off engines near landing to save fuel
            debug("Power OFF!");
            pow = 0;
        }
